...
<script>
    ...
    async function buscarHistorico() {
        ...
        try {
            // Chama a NOVA action segura
            const res = await fetch(`/api_dados.php?action=get_history_positions&deviceId=${selectedDeviceId}&date=${date}`);
            const data = await res.json();
            list.innerHTML = '';
            
            if(!data || !data.length) { 
                list.innerHTML = '<div class="text-center text-slate-600 pt-10">Nenhum dado encontrado.</div>'; 
                return; 
            }
            ...
            console.error(e);
            list.innerHTML = '<div class="text-center text-red-500 pt-10">Erro ao buscar dados.</div>'; 
        }
    }
