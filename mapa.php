...
// --- 2. BUSCA VEÍCULOS, ÍCONES E LISTA BASE DO SISTEMA ---
// Precisamos garantir que TODOS os veículos do banco apareçam, mesmo sem transmissão
$sql = "
    SELECT v.id, v.name, v.plate, v.traccar_device_id, v.category, i.url as icon_url, c.name as client_name
...
// Mapas auxiliares para o Frontend
$vehiclesData = []; // Para ícones [device_id => url]
$customerData = []; // Para nomes de clientes [device_id => nome]
$allowedIds = [];   // IDs permitidos do Traccar

foreach ($dbVehicles as $v) {
    if ($v['traccar_device_id']) {
        $tid = $v['traccar_device_id'];
        $allowedIds[] = (int)$tid;
        if ($v['icon_url']) $vehiclesData[$tid] = $v['icon_url'];
        if ($v['client_name']) $customerData[$tid] = $v['client_name'];
    }
}

$jsIconMap = json_encode($vehiclesData);
$jsCustomerMap = json_encode($customerData);
$jsAllowedIds = json_encode($allowedIds);

// --- 3. BUSCA MOTORISTAS VINCULADOS ---
$jsCurrentDrivers = '{}';
try {
    ...
    if ($check) {
        $sqlCD = "SELECT v.traccar_device_id, d.name 
                  ...
                  WHERE v.tenant_id = ?";
        // Reutiliza logica de permissão se necessário (simplificado aqui para tenant)
        // ... (Para manter o código limpo, assumindo filtro por tenant + allowedIds no JS)
        $stmtCD = $pdo->prepare($sqlCD);
        $stmtCD->execute([$tenant_id]);
        $jsCurrentDrivers = json_encode($stmtCD->fetchAll(PDO::FETCH_KEY_PAIR));
    }
} catch(Exception $e) { /* Ignora */ }

// --- 4. DATA PRE-FETCHING (TRACCAR) ---
// Busca dados diretamente do Traccar via PHP para renderização inicial (SSR)
...
// Extrai JSESSIONID para WebSocket
...
// 4.2 Busca Devices e Positions Iniciais
function fetchTraccar($url) {
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_COOKIEFILE, '/tmp/traccar_cookie.txt');
...
$traccarDevices = fetchTraccar("$TRACCAR_HOST/devices");
$traccarPositions = fetchTraccar("$TRACCAR_HOST/positions");

// 4.3 Mesclagem Inteligente (DB + Traccar)
// Cria a lista inicial garantindo que o que está no DB apareça, mesmo sem dados no Traccar
$initialDevices = [];
$initialPositions = [];

// Indexa dados do Traccar para busca rápida
$traccarDevMap = [];
foreach ($traccarDevices as $td) { $traccarDevMap[$td['id']] = $td; }

$traccarPosMap = [];
foreach ($traccarPositions as $tp) { $traccarPosMap[$tp['deviceId']] = $tp; }

// Monta lista final
foreach ($dbVehicles as $dbV) {
    $tid = $dbV['traccar_device_id'];
    if (!$tid) continue;

    // Dados do Traccar ou Default (Offline)
    $tData = $traccarDevMap[$tid] ?? [
        'id' => $tid,
        'name' => $dbV['name'], // Usa nome do DB se Traccar não tiver
        'uniqueId' => 'N/A',
        'status' => 'offline',
        'lastUpdate' => null,
        'disabled' => false
    ...
    // Força o nome do DB para manter consistência visual
    $tData['name'] = $dbV['name']; 
    
    $initialDevices[] = $tData;

    // Se tiver posição, adiciona
    if (isset($traccarPosMap[$tid])) {
        $initialPositions[] = $traccarPosMap[$tid];
    }
}

$jsInitialDevices = json_encode($initialDevices);
$jsInitialPositions = json_encode($initialPositions);
...
<div class="relative w-full h-full overflow-hidden flex flex-col">
    ...
    <div class="absolute top-4 left-4 z-[500] w-72">
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-2 flex items-center gap-2">
            ...
            <div id="ws-status" class="status-dot status-off" title="Status Conexão"></div>
        </div>
    </div>
...
<script>
    // --- CONFIGURAÇÃO E DADOS INICIAIS (SSR) ---
    const WS_URL = "<?php echo $wsUrl; ?>";
    const SESSION_TOKEN = "<?php echo $sessionToken; ?>"; 
    
    // Dados injetados pelo PHP (Estado Inicial Completo)
    ...
    let lastDevices = <?php echo $jsInitialDevices ?: '[]'; ?>;
    let initialPositions = <?php echo $jsInitialPositions ?: '[]'; ?>;
    ...
    function startPolling() { 
        ...
        document.getElementById('ws-status').className = "status-dot status-http"; 
        // Não chamamos loadDataHttp aqui para não piscar a tela, o renderMap já cuidou do inicial
        pollingInterval = setInterval(loadDataHttp, 10000); // Polling mais lento, só backup
    }
    ...
    function connectWS() {
        ...
        try {
            ...
            socket.onopen = function() { console.log('WS Conectado'); statusEl.className = "status-dot status-ws"; stopPolling(); };
            socket.onmessage = function(event) {
                const data = JSON.parse(event.data);
                if (data.devices) {
                    const filteredDevs = data.devices.filter(d => allowedDeviceIds.includes(d.id));
                    if(filteredDevs.length > 0) { 
                        // Update apenas, mantendo os que já existem no lastDevices (que contem os do DB)
                        lastDevices = updateArray(lastDevices, filteredDevs); 
                        renderMap(); 
                    }
                }
                if (data.positions) {
            ...
            socket.onclose = function() { 
                ...
                setTimeout(connectWS, 3000); // Tenta reconectar WS
            };
            socket.onerror = function() {
                startPolling(); // Falhou WS, vai pro Polling
            }
    ...
    async function loadDataHttp() {
        try {
            ...
            if(devRes.ok && posRes.ok) {
                ...
                const filteredDevs = devs.filter(d => allowedDeviceIds.includes(d.id));
                lastDevices = updateArray(lastDevices, filteredDevs);
    ...
    function renderMap() {
        ...
        lastDevices.forEach(d => {
            // Garante que só mostre o que é permitido
            ...
            // Tratamento para veículos SEM posição (Nunca transmitiu)
            const p = positionsMap[d.id]; 
            
            let speed = '-', dateFull = 'Sem Sinal', addr = 'Aguardando atualização...';
            let lat = null, lon = null, course = 0;
            let ignHtml = '<span class="text-gray-300">-</span>';
            let pwrHtml = '<span class="text-gray-300">-</span>';
            let batHtml = '<span class="text-gray-300">-</span>';

            if (p) {
                speed = (p.speed * 1.852).toFixed(0);
                const dateObj = new Date(p.fixTime);
                dateFull = dateObj.toLocaleString('pt-BR'); 
                lat = p.latitude;
                lon = p.longitude;
                course = p.course;
                
                ignHtml = st.ignition ? '<span class="text-green-600 font-bold text-[10px]">ON</span>' : '<span class="text-gray-400 text-[10px]">OFF</span>';
                
                // Endereço
                const k = lat.toFixed(4)+','+lon.toFixed(4);
                if(!geoCache[k]) geoQueue.push({id:d.id, lat:lat, lon:lon});
                addr = geoCache[k] || '...';
                ...
                if (st.battery !== undefined && st.battery !== null) {
                    ...
                    batHtml = bVal > 20 
                        ? `<div class="flex items-center justify-center gap-1 text-green-600 font-bold text-xs"><i class="fas fa-battery-full"></i> ${bVal}%</div>`
                        : `<div class="flex items-center justify-center gap-1 text-red-500 font-bold text-xs animate-pulse"><i class="fas fa-battery-empty"></i> ${bVal}%</div>`;
                }
                if (st.power !== undefined && st.power !== null) {
                    const pVal = parseFloat(st.power);
                    pwrHtml = (pVal < 1 || st.alarm === 'powerCut')
                        ? `<div class="flex items-center justify-center gap-1 text-red-500 font-bold text-xs"><i class="fas fa-plug"></i> OFF</div>`
                        : `<div class="flex items-center justify-center gap-1 text-green-600 font-bold text-xs"><i class="fas fa-plug"></i> ${pVal.toFixed(1)}V</div>`;
                }
            }
            ...
            const proto = (p && p.protocol) ? p.protocol.toUpperCase() : '-';
            ...
            // Só adiciona evento de clique se tiver latitude
            const clickEvent = lat ? `onclick="focusDev(${lat}, ${lon}, ${d.id})"` : '';

            // GRID ROW
            html += `<tr id="row-${d.id}" ${clickEvent} class="transition border-b ${isSel}">
            ...
            if(lat) {
                ...
                if(followingId === d.id) map.panTo([lat, lon], {animate:true, duration:0.5});
            }
        });
    ...
    function openLockModal(e, id, name, isBlocked) { e.stopPropagation(); document.getElementById('modal-security').classList.remove('hidden'); document.getElementById('sec-dev-id').value = id; document.getElementById('sec-veh-name').innerText = name; const btn = document.getElementById('btn-sec-confirm'); document.getElementById('sec-cmd-type').value = isBlocked ? 'unlock' : 'lock'; btn.className = isBlocked ? "w-full bg-green-600 text-white font-bold py-2 rounded shadow hover:bg-green-700" : "w-full bg-red-600 text-white font-bold py-2 rounded shadow hover:bg-red-700"; btn.innerText = isBlocked ? "Confirmar Desbloqueio" : "Confirmar Bloqueio"; document.getElementById('sec-cmd-name').innerText = isBlocked ? "Desbloquear" : "Bloquear"; document.getElementById('sec-password').value = ''; document.getElementById('sec-password').focus(); }
    ...
    // 1. Processa posições injetadas pelo PHP
    updatePositions(initialPositions);
    // 2. Renderiza o mapa imediatamente
    renderMap();
    if(initialPositions.length > 0) fitAll();
    
    // 3. Inicia conexões dinâmicas
    connectWS();
</script>